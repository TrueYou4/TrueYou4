export default async function handler(req, res) {
  ...
  // Your HTML content with inline CSS
  const htmlContent = `
    <!DOCTYPE html>
    <html>
  ...
  // Sending the email through Brevo API
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'api-key': process.env.BREVO_API_KEY, // Make sure you have set the Brevo API Key in your environment variables
    },
    body: JSON.stringify({
      sender: { name: "TrueYou", email: "trueyoupro@gmail.com" }, // Sender email
      to: [{ email: email, name: name }], // Recipient's email and name
